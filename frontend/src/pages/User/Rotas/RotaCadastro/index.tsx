...
const RotaCadastro: React.FC<RotaCadastroProps> = ({ initialLocations, isAdmin = false, isGuia = false }) => {
		...
		<div className="w-[95vw] flex flex-col items-center min-h-screen bg-white pt-[5vh] pb-[15vh]">
			<div className="w-full max-w-md lg:max-w-5xl px-4 sm:px-6">
				<div className="w-full flex items-center mb-6">
					<button onClick={() => window.history.back()}>
						<IconArrowChevron
							class={`${styled.icon} ${styled.arrow}`}
						/>
					</button>
				</div>
			</div>
			...
			<div className="w-[100%] max-w-md lg:max-w-5xl px-6 flex flex-col items-center">
				...
				{routeImage ? (
					...
					>
						<IconPinSharp class={`${styled.icon} ${styled.pin}`} mainColor={userData.color} />
					</div>
				)}
				...
				<form onSubmit={handleSubmit} className={`${styled.form} w-full flex flex-col items-center`}>
					<div className="w-full flex flex-col lg:flex-row lg:gap-12 lg:items-start">
						<div className="w-full lg:w-1/2 flex flex-col">
							<h2
								className="text-3xl font-bold w-full my-4 text-start"
								style={{ color: userData.color }}
							>
								Informações Gerais
							</h2>
								...
								<label className="flex items-center cursor-pointer">
									<Checkbox
										...
										sx={{
											paddingLeft: 0,
											color: '#b6b6b6ff',
											'&.Mui-checked': { color: userData.color },
											'& .MuiTouchRipple-root': { color: userData.color },
										}}
									/>
									Criar Rota Privada
								</label>
								<p className="text-sm text-gray-500 text-start">
									Essa ficará disponível somente para você
								</p>
							</div>
						</div>
						...
						<div className="w-full lg:w-1/2 flex flex-col mt-6 lg:mt-0">
									...
									<TextField
										...
										sx={{
											'& .MuiOutlinedInput-root': {
												borderRadius: '10px',
												'& .MuiOutlinedInput-notchedOutline': { borderColor: userData.color, borderWidth: '2px' },
												'&:hover .MuiOutlinedInput-notchedOutline': { borderColor: userData.color, borderWidth: '2px' },
												'&.Mui-focused .MuiOutlinedInput-notchedOutline': { borderColor: userData.color, borderWidth: '2px' },
											},
											'& .MuiInputLabel-root': {
												color: userData.color,
												'&.Mui-focused': { color: userData.color },
											},
										}}
									/>
								)}
							...
							{selectedLocations.length > 1 && (
								<div className="w-full text-start my-3">
									<p>
										<span style={{ color: userData.color, fontWeight: 'bold' }}>Segure </span>e{' '}
										<span style={{ color: userData.color, fontWeight: 'bold' }}>Arraste</span> para organizar a sequência:
									</p>
								</div>
							)}
